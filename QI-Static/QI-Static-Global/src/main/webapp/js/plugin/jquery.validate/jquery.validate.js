...
$.validator = function( options, form ) {
	...
	$.each( this.settings.rules, function( key, value ) {
		var rules = $.validator.normalizeRule( value );
		if( rules.required ){
			$(form).find("[name='" + key + "']").parent().append('<div class="asterisk"></div>');
		}
	});
};
	...
	prototype: {
		...
		formatAndAdd: function( element, rule ) {
			...
			if ( rule.method === "minlength" || rule.method === "maxlength" ){
				rule.parameters = rule.parameters / 2;
			}
		...
		showLabel : function(element, message) {
            if (message) message = '<span class="msg-wrap n-error"><span class="n-arrow"><b>◆</b><i>◆</i></span><span class="n-icon"></span><span class="n-msg">' + message + '</span></span>';
		...
		getLength: function( value, element ) {
			...
			case "select":
				return $( "option:selected", element ).replace(/[^\x00-\xff]/g, "**").length;
			case "input":
				if ( this.checkable( element ) ) {
					return this.findByName( element.name ).filter( ":checked" ).replace(/[^\x00-\xff]/g, "**").length;
				}
			}
...
jQuery.validator.addMethod("greater", function (value, element, param) {
    var target = $(param);
    return (Number(value) == 0 && Number(target.val()) == 0) || $.trim(target.val()) == '' || Number(value) > Number(target.val());
}, "比较错误");

// 比较大于等于
jQuery.validator.addMethod("greaterEqual", function (value, element, param) {
	var target = $(param);
	return $.trim(target.val()) == '' || Number(value) >= Number(target.val());
}, "比较错误");
...
// 比较小于等于
jQuery.validator.addMethod("lessEqual", function (value, element, param) {
	var target = $(param);
	return $.trim(target.val()) == '' || Number(value) <= Number(target.val());
}, "比较错误");
